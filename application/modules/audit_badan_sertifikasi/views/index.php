				...
				<div class="card-header">
					...
					<div class="mt-4 float-right ">
						<button type="button" class="btn btn-primary" id="add" title="Add New Scope">
							<i class="fa fa-plus mr-1"></i>New Instansi
						</button>
					</div>
				</div>
				<div class="card-body">
								...
								<tbody>
									<?php if ($data) foreach ($data as $k => $v) : $k++; ?>
										<tr>
											...
											<td><?= $v->description; ?></td>
											<td>
												<?php if ($ArrAuditor[$v->id]) : ?>
													<li><?= implode("</li><li>", $ArrAuditor[$v->id]); ?></li>
												<?php endif; ?>
											</td>
...
<script>
	$(document).ready(function() {
		...
		/* Add Standard */
		$(document).on('click', '#add-auditor', function() {
			let html = '';
			let n = $('#tblAuditor tbody tr').length + 1

			html += `
				<tr data-row="${n}">
					<td class="number text-center">${n}</td>
					<td class="">
						<input type="text" name="auditor[${n}][name]" class="form-control required" placeholder="Auditor Name">
					</td>
					<td class="text-center">
						<button type="button" class="btn btn-xs btn-danger btn-icon del-auditor"><i class="fa fa-trash" aria-hidden="true"></i></button>
					</td>
				</tr>`;
			$('#tblAuditor tbody').append(html)
		})

		$(document).on('click', '.del-auditor', function() {
			const id = $(this).data('id')
			const btn = $(this)
			if (id) {
				Swal.fire({
					...
					if (value.isConfirmed) {
						$.ajax({
							url: siteurl + active_controller + 'delete_auditor',
							data: {
								id
							},
							type: 'POST',
	...
	function renumber() {
		let n = 0
		$('#tblAuditor tbody tr').each(function() {
			n++;
			$(this).find('td:first').text(n)
			$(this).find('input[name^="auditor"]').attr('name', "auditor[" + n + "][id]")
			$(this).find('input[name^="auditor"]').attr('name', "auditor[" + n + "][name]")
		})
	}
