<div class="box box-primary">
    ...
    <?= form_open($this->uri->uri_string(),array('id'=>'frm_users','name'=>'frm_users','role'=>'form','class'=>'form-horizontal')) ?>
    	<div class="box-body">
			...
			<div hidden class="form-group <?= form_error('st_aktif') ? ' has-error' : ''; ?>">
			    <label for="st_aktif" class="col-md-2 control-label"><?= lang('jabatan') ?></label>
				     <?php

                                  $isi = isset($data->username) ? $data->username : '';								 
								  if($isi !='') {
								  $users	= $this->db->query("SELECT * FROM tbl_jabatan")->result();
								    echo  "<div class='col-md-3'>";
									echo "<select id='id_jabatan' name='id_jabatan' class='select2'>
											<option value=''>Pilih Jabatan</option>";
											foreach($users as $pic){
									if($data->id_jabatan == $pic->id ){
									$selected='selected';
									}
									else{
									$selected='';	
									}
									echo "<option value='$pic->id' $selected>$pic->nm_jabatan</option>";
											}
									echo "</select>"; 
									echo "</div>";
								  }
								 ...
								 ?>
			        <div class="col-md-3" id="select_jabatan">
						<select class="form-control input-sm select2" name="id_jabatan" id="id_jabatan">
						<option value="">Pilih Jabatan</option>								
						</select> 
					</div>
					            <?php 
