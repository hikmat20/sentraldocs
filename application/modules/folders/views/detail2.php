			...
			<tbody>
				...
				if ($row) {
					...
					foreach ($row as $datas) {
						...
						$dtid = $datas->id;
						$appr = $this->db->query("SELECT approval_on as tgl_dt, keterangan as ket_dt FROM tbl_approval WHERE id_dokumen ='$dtid' AND nm_table='gambar2'")->result_array();
				...
				?>
						...
						if ($ENABLE_MANAGE) {	?>
							<a href="#" onClick="window.open('<?php echo site_url(); ?>assets/files/<?php echo "$datas->nama_file"; ?>', '_blank')" class="btn btn-sm btn-success" title="View Data"><i class="fa fa-eye"></i><a />
								...
								<a href="#" class="btn btn-sm btn-warning edit" title="Revisi Data" data-file="<?php echo $datas->nama_file ?>" data-id="<?php echo $datas->id ?>" data-table="gambar2"><i class="fa fa-edit"></i><a />
